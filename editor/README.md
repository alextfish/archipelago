...
- The editor now uses **0-based grid coordinates**.
- Valid `x` values are `0` to `width - 1`.
- Valid `y` values are `0` to `height - 1`.
...
```json
{
  ...
  "islands": [
    { "id": "A", "x": 0, "y": 0 },
    { "id": "B", "x": 0, "y": 3, "constraints": ["num_bridges=3"] }
  ],
  "bridgeTypes": [
  ...
  "constraints": [
    ...
    { "type": "MustTouchAHorizontalBridge", "params": { "x": 1, "y": 1 } }
  ],
  "maxNumBridges": 10
}
```
