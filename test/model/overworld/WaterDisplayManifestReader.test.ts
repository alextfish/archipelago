...
describe('WaterDisplayManifestReader', () => {
  it('pairs sibling waterflow/water layers and sets animation metadata from visual water tiles', () => {
    ...
    expect(tile1?.visualIsDirectionOnly).toBe(true);
  ...
  it('uses water as logic fallback when waterflow layer is absent', () => {
    const tiledMapData = {
      width: 1,
      height: 1,
      layers: [{
    ...
    expect(tile?.logicLayerName).toBe('River/water');
    expect(tile?.visualHasFlowDirections).toBe(true);
