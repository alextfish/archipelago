...
describe("IslandPassingBridgeCountConstraint.getDisplayItems", () => {
  it("returns 'good' when count is correct, with requiredCount, compassFrame and conversationVariables", () => {
    ...
    expect(items).toEqual([{ elementID: "A", glyphMessage: "good", constraintType: "IslandPassingBridgeCountConstraint", requiredCount: 1, compassFrame: 0, conversationVariables: { count: "1", direction: "above" } }]);
  });
  ...
  it("returns 'not-enough bridge above island' when too few pass above", () => {
    ...
    expect(items).toEqual([{ elementID: "A", glyphMessage: "not-enough bridge above island", constraintType: "IslandPassingBridgeCountConstraint", requiredCount: 1, compassFrame: 0, conversationVariables: { count: "1", direction: "above" } }]);
  });
  ...
  it("returns 'too-many bridge below island' when too many pass below", () => {
    ...
    expect(items).toEqual([{ elementID: "A", glyphMessage: "too-many bridge below island", constraintType: "IslandPassingBridgeCountConstraint", requiredCount: 1, compassFrame: 2, conversationVariables: { count: "1", direction: "below" } }]);
  });
  ...
  it("returns 'not-enough bridge left-of island' when too few pass to the left", () => {
    ...
    expect(items).toEqual([{ elementID: "A", glyphMessage: "not-enough bridge left-of island", constraintType: "IslandPassingBridgeCountConstraint", requiredCount: 1, compassFrame: 3, conversationVariables: { count: "1", direction: "left-of" } }]);
  });
  ...
  it("returns 'not-enough bridge right-of island' when too few pass to the right", () => {
    ...
    expect(items).toEqual([{ elementID: "A", glyphMessage: "not-enough bridge right-of island", constraintType: "IslandPassingBridgeCountConstraint", requiredCount: 1, compassFrame: 1, conversationVariables: { count: "1", direction: "right-of" } }]);
  });

  it("returns 'not-enough bridge adjacent island' with compassFrame 5 for adjacent mode", () => {
    ...
    expect(items).toEqual([{ elementID: "A", glyphMessage: "not-enough bridge adjacent island", constraintType: "IslandPassingBridgeCountConstraint", requiredCount: 1, compassFrame: 5, conversationVariables: { count: "1", direction: "adjacent" } }]);
  });
  ...
  it("includes the expected count as requiredCount in all display items", () => {
    const islands = [{ id: "A", x: 2, y: 5 }];
    const puzzle = makeMockPuzzle({ islands, bridges: [], placedBridges: [] });

    const constraint = new IslandPassingBridgeCountConstraint("A", "above", 3);
    const items = constraint.getDisplayItems(puzzle as any);

    expect(items[0].requiredCount).toBe(3);
  });
});
