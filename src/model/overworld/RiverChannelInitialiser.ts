 ...
 * pure helpers for building and merging the water grid used by the
 * overworld water-propagation system.
 *
 * All public methods are static; the class holds no state.
...
export class RiverChannelInitialiser {
   ...
   *     the map data is absent.
   */
  static buildMergedWaterLayer(tiledMapData: any): number[] | undefined {
    if (!tiledMapData) return undefined;
    ...
    const waterLayers = TiledLayerUtils.findTileLayersByName(tiledMapData.layers ?? [], 'waterflow');
    console.log(`[MergedWater] Merging ${waterLayers.length} waterflow layer(s) into master grid`);
  ...
  /**
   * Infer the cardinal edge direction of a flow-square that sits on the border
   * of a puzzle region.
   *
   * @param lx - Local X within the puzzle (0-based).
   * @param ly - Local Y within the puzzle (0-based).
   * @param width - Width of the puzzle in tiles.
   * @param height - Height of the puzzle in tiles.
   */
  static inferEdgeDirection(
    ...
    // Fallback for interior tiles — shouldn't occur for valid border outputs
    return 'E';
  }
}
